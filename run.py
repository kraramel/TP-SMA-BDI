...
def run_jam_agent(facts_file):
  shell_command = 'java -classpath jam.jar com.irs.jam.JAM \'{0}\' \'agent.jam\''.format(facts_file)
...
def vizualize(xmax, ymax, base_x, base_y, rocks, movements, events, save=True):
  ...
  ln, = plt.plot([], [], 'ro', animated=True, label='Agent')
  water_rock_x = [i[0] for i in rocks if i[2]==1]
  water_rock_y = [i[1] for i in rocks if i[2]==1]
  water_sc = ax.scatter(water_rock_x,water_rock_y,s=10,c="g",marker="x",label="Roche contenant de l'eau")
  ax.scatter([i[0] for i in rocks if i[2]==0],[i[1] for i in rocks if i[2]==0],s=10,c="b",marker="x",label='Roche vide')
  def init():
  ...
  if save:
    ani.save(f'./{len(rocks)}_{xmax}_{ymax}_asimulation.gif', fps=35, writer='imagemagick')
  else:
    plt.show()
